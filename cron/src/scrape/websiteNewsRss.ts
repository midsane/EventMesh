...
import {
  websiteRssFeeds,
  websiteSourceMapForBetterName,
  ytLimitFromEachSource,
} from "../constant.js";
...
const parser = new Parser({
  customFields: {
    item: [
      ["media:thum>bnail", "mediaThumbnail"],
...
export const scrapeWebsitNews = async () => {
  ...
  for (let feedNumber = 0; feedNumber < websiteRssFeeds.length; feedNumber++) {
    ...
    try {
      ...
      if (articleTaken > 0)
        console.log(
          `fetched ${articleTaken} articles from ${extractTitle(feed.title)}".`,
        );
    } catch (err) {
  ...
  const writePath =
    mode === "development" ? "articles.json" : "./cron/articles.json";

...
const writePath =
  mode === "development"
    ? "twitter-articles.json"
    : "./cron/twitter-articles.json";
    ...
    try {
      ...
      $(".timeline-item")
        ...
        .each((_, el) => {
          ...
          if (
            !tweetNews.title ||
            tweetNews.title.length < 15 ||
            tweetNews.title.startsWith("Enable hls") ||
            !tweetNews.link ||
            !tweetDate
          ) {
            console.log(
              "Skipping tweet due to missing or invalid data:",
              tweetNews,
            );
            return;
          }
      ...
      console.log(
        `Finished scraping ${source}. Total tweets collected: ${allTweets.length}`,
      );
    } catch (err) {

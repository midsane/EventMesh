...
export const processArticle = async (article: Article) => {
  ...
  if (alreadyexistNews) {
    console.log(
      `---Skipping processing of article as it already exists in DB: ${article.title}`,
    );
    return;
  }
  ...
  if (article.content) queryText += `${article.content}`;

  try {
    console.log(
      `--Searching for similar articles in vector DB with query: ${queryText}`,
    );
    ...
    console.log(
      `--Found ${topNewsChunks.length} similar articles in vector DB.`,
    );
    if (topNewsChunks.length === 0) {
      matchType = MatchType.UNRELATED;
      console.log(
        "--No similar news found for this article in vector DB, treating as new article.",
      );
    } else {
  ...
  if (!matchType) {
    console.log(
      `---Unknown match type/invalid related id skipping article processing.`,
    );
    return;
  }
  ...
  if (
    matchType === MatchType.UNRELATED &&
    (article.youtube || article.twitter)
  ) {
  ...
  const miniNewsId = await handleArticle(
    article,
    category,
    matchType,
    relatedId,
  ...
  if (!miniNewsId) {
    console.log(
      `---Skipping processing of article due to error in storing it in DB.`,
    );
    return;
  }
...
export const processNewsFromFile = async (jsonName: string) => {
  const articlePath =
    mode === "development" ? `./${jsonName}` : `./cron/${jsonName}`;
  try {
    ...
    for (let i = 0; i < articles.length; i++) {
      ...
      switch (jsonName) {
        ...
        case "all_articles.json":
          canWeProcessArticle =
            article.link && article.title && article.source ? true : false;
          break;
      ...
      try {
        if (canWeProcessArticle) {
          ...
          console.log(
            `---Skipping processing of article due to missing required fields.`,
          );
        }
      } catch (error) {
        console.log(
          `Error processing article ${article.title}: ${article.link}`,
          error,
        );
      }
    }

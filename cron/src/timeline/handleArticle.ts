...
import {
  TwitterNewsAddedInDB,
  WebsiteNewsAddedInDB,
  YoutubeNewsAddedInDB,
} from "../util/logsForDebugging.js";

const prisma = new PrismaClient();

const createMiniNews = async (
  article: Article,
  category: string,
  newsId: string,
) => {
  ...
  // we don't have proper to bias model yet.
...
const handleTimelineArticle = async (
  ...
  if (!relevantArticle) {
    console.warn(
      "---Related article not found in DB, skipping child creation.",
    );
    return null;
  }

  try {
    const miniNewsId = await createMiniNews(
      article,
      categories,
      relevantArticle.newsId,
    );
...
const handleSameEventArticle = async (article: Article, relatedId: string) => {
  const consoleError = console.error;
  console.error = () => {};
  ...
  try {
    ...
    console.log(
      "Updated existing article with new link, all links are:",
      miniNews.links,
      "\n newsid:",
      miniNews.newsId,
    );
...
const article = {
  ...
  title:
    "Mohit Suri, Ahaan Panday and Aneet Padda’s ‘Saiyaara’ becomes 25th biggest hit of Hindi cinema; records its lowest collection on Monday",
  content: "",
  source: "TOI",
  pubDate: new Date(),
  imageUrl: "",
